...
public final class InventoryInteractionsKeybindRegression {
	...
	private static void runCraftingPlayerInventorySortKeybindRegression() {
		...
		waitForClientScreen("prepared crafting table",
				() -> Minecraft.getInstance().screen instanceof CraftingScreen screen && screen.getMenu().containerId == containerId
						&& screen.getMenu().slots.get(10).getItem().is(Items.COBBLESTONE) && screen.getMenu().slots.get(10).getItem().getCount() == 1
						&& screen.getMenu().slots.get(11).getItem().is(Items.COBBLESTONE) && screen.getMenu().slots.get(11).getItem().getCount() == 2
						&& screen.getMenu().slots.stream().filter(Slot::hasItem).count() == 2 && screen.getMenu().getCarried().isEmpty());
		waitForServerCondition("prepared crafting table",
				player -> player.containerMenu instanceof CraftingMenu menu && menu.containerId == containerId
						&& menu.slots.get(10).getItem().is(Items.COBBLESTONE) && menu.slots.get(10).getItem().getCount() == 1
						&& menu.slots.get(11).getItem().is(Items.COBBLESTONE) && menu.slots.get(11).getItem().getCount() == 2
						&& menu.slots.stream().filter(Slot::hasItem).count() == 2 && menu.getCarried().isEmpty());
		boolean handled = runOnClient(() -> {
			if (!(Minecraft.getInstance().screen instanceof CraftingScreen screen)) {
				throw new IllegalStateException("Crafting screen is not open for the player sort control");
			}
			Slot playerMainTopRight = screen.getMenu().getSlot(18);
			double x = screen.getGuiLeft() + playerMainTopRight.x + 11.0;
			double y = screen.getGuiTop() + playerMainTopRight.y - 8.0;
		...
		requireHandled(handled, "Crafting-table player inventory sort control was not handled");
		try {
			waitForPlayerInventorySort("crafting-table player inventory sort");
		} catch (IllegalStateException e) {
			throw new IllegalStateException(e.getMessage() + ": " + runOnServer(player -> "cobblestone=" + countItems(player, Items.COBBLESTONE) + ", stacks="
					+ countStacks(player, Items.COBBLESTONE) + ", menu=" + player.containerMenu.getClass().getSimpleName()), e);
		}
	}
