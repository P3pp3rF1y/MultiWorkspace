...
public class EmiRecipeViewerAutomation implements RecipeViewerAutomation {
    ...
    public String stateJson() {
        ...
        return "{\"ok\":true,"
                ...
                + "\"recipeCount\":" + EmiApi.getRecipeManager().getRecipes().size() + ","
    ...
    public String openJson(String requestJson) {
        ...
        return "{\"ok\":true,"
                + JsonUtil.property("viewer", id()) + ","
                + JsonUtil.property("item", stack.get().getId().toString()) + ","
    ...
    public String queryJson(String requestJson) {
        ...
        return "{\"ok\":true,"
                + JsonUtil.property("viewer", id()) + ","
                + JsonUtil.property("item", stack.get().getId().toString()) + ","
                + JsonUtil.property("name", stack.get().getName().getString()) + ","
                + "\"uiBacked\":true,"
    ...
    private static Optional<EmiStack> findStack(JsonObject request) {
        ...
        Optional<EmiStack> exact = EmiApi.getIndexStacks().stream()
                .filter(stack -> stack.getId().toString().equals(normalized))
                .filter(stack -> focus.map(selector -> !stack.getItemStack().isEmpty() && RecipeViewerStackMatcher.matches(stack.getItemStack(), selector)).orElse(true))
                .findFirst();
        if (exact.isPresent()) {
            return exact;
        }
        return EmiApi.getIndexStacks().stream()
                .filter(stack -> stack.getName().getString().toLowerCase(Locale.ROOT).equals(normalized))
                .filter(stack -> focus.map(selector -> !stack.getItemStack().isEmpty() && RecipeViewerStackMatcher.matches(stack.getItemStack(), selector)).orElse(true))
                .findFirst();
    }
    ...
    private static List<EmiRecipe> openedRecipes(EmiRecipeManager manager, EmiStack stack, boolean usages) {
        ...
        try {
            ...
            if (!recipes.isEmpty()) {
                ...
                List<EmiRecipe> filteredScreenRecipes = recipes.stream()
                        .filter(recipe -> itemStack.isEmpty() || containsRecipeStack(recipe, itemStack, usages))
    ...
    private static boolean containsStack(List<? extends dev.emi.emi.api.stack.EmiIngredient> ingredients, ItemStack stack) {
    ...
    private static boolean stacksMatch(ItemStack candidate, ItemStack focusedStack) {
        if (ItemStack.isSameItemSameTags(candidate, focusedStack)) {
            return true;
        }
        return (!focusedStack.hasTag() || !candidate.hasTag()) && ItemStack.isSameItem(candidate, focusedStack);
    }

    private static String recipeJson(EmiRecipe recipe, ItemStack focusedStack) {
        EmiDisplaySlots displaySlots = displaySlots(recipe);
        List<? extends dev.emi.emi.api.stack.EmiIngredient> inputs = displaySlots.inputs().isEmpty() ? recipe.getInputs() : displaySlots.inputs();
        List<? extends dev.emi.emi.api.stack.EmiIngredient> outputs = displaySlots.outputs().isEmpty() ? recipe.getOutputs() : displaySlots.outputs();
        return "{"
                + JsonUtil.property("id", recipe.getId() == null ? null : recipe.getId().toString()) + ","
                + JsonUtil.property("category", recipe.getCategory().getId().toString()) + ","
                + JsonUtil.property("categoryName", recipe.getCategory().getName().getString()) + ","
                + "\"inputCount\":" + inputs.size() + ","
                + "\"outputCount\":" + outputs.size() + ","
                + "\"inputs\":" + ingredientsJson(inputs) + ","
                + "\"outputs\":" + ingredientsJson(outputs)
                + "}";
    }
    ...
    private static String ingredientJson(EmiIngredient ingredient) {
        return RecipeJsonUtil.ingredientJson(ingredient.getEmiStacks().stream()
                .map(stack -> stack.getItemStack().isEmpty() ? RecipeJsonUtil.stackJson("entry", stack.getId().toString(), stack.getName().getString(), stack.getAmount(), stack.getChance()) : RecipeJsonUtil.itemStackJson(stack.getItemStack()))
                .collect(Collectors.toList()));
    }
