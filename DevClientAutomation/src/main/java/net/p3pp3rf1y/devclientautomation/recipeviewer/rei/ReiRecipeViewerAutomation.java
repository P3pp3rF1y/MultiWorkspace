...
public class ReiRecipeViewerAutomation implements RecipeViewerAutomation {
    ...
    public String stateJson() {
        ...
        return "{\"ok\":true,"
                ...
                + "\"recipeCount\":" + DisplayRegistry.getInstance().displaySize() + ","
    ...
    private static Optional<EntryStack<?>> findStack(JsonObject request) {
        ...
        Optional<EntryStack<?>> byName = EntryRegistry.getInstance().getEntryStacks()
                .filter(ReiRecipeViewerAutomation::isItemStack)
        ...
        return BuiltInRegistries.ITEM.getOptional(new ResourceLocation(itemId))
                .map(ReiRecipeViewerAutomation::entryStack);
    }

    private static EntryStack<?> entryStack(Item item) {
        try {
            return EntryStacks.of(new ItemStack(item));
    ...
    private static String displayKey(Display display) {
        return display.getCategoryIdentifier().getIdentifier() + "|" + display.getDisplayLocation().map(ResourceLocation::toString).orElse(String.valueOf(System.identityHashCode(display)));
    }
    ...
    private static List<? extends Display> generatedDisplays(DynamicDisplayGenerator<?> generator, EntryStack<?> stack, boolean usages, boolean allowItemFallback) {
        ...
        List<? extends Display> globalDisplays = generator.generate(ViewSearchBuilder.builder()).orElse(List.of()).stream()
                .filter(display -> containsStack(usages ? display.getInputEntries() : display.getOutputEntries(), stack, allowItemFallback))
                .toList();
        if (focusedDisplays.isEmpty()) {
            return globalDisplays;
        }
        Set<String> focusedIds = focusedDisplays.stream().map(ReiRecipeViewerAutomation::displayKey).collect(Collectors.toCollection(HashSet::new));
        return java.util.stream.Stream.concat(focusedDisplays.stream(), globalDisplays.stream().filter(display -> !focusedIds.contains(displayKey(display)))).toList();
    }
    ...
    private static boolean stacksMatch(EntryStack<?> candidate, EntryStack<?> focusedStack, boolean allowItemFallback) {
        ...
        return (!focusItemStack.hasTag() || !candidateStack.hasTag()) && ItemStack.isSameItem(candidateStack, focusItemStack);
    }
    ...
    private static String recipeJson(Display display) {
        return "{"
                + JsonUtil.property("id", display.getDisplayLocation().map(ResourceLocation::toString).orElse(null)) + ","
