...
public class ReiRecipeViewerAutomation implements RecipeViewerAutomation {
	...
	private static boolean hasColorComponents(ItemStack stack) {
		...
		return mainColor != null && mainColor != BackpackWrapper.DEFAULT_MAIN_COLOR
				|| accentColor != null && accentColor != BackpackWrapper.DEFAULT_ACCENT_COLOR;
	}
			...
			minecraft.getSingleplayerServer().getRecipeManager().getRecipes().forEach(recipe -> {
				...
				for (RecipeDisplayEntry displayEntry : displayEntries) {
					try {
						...
						if (!hasSophisticatedBackpackInput(entry)
								&& containsStack(usages ? entry.inputs() : entry.outputs(), focusedStack, allowItemFallback, focusSelector)) {
							entries.add(entry);
						}
	...
	private static List<RecipeEntry> commonBackpackCatalogEntries(ItemStack focusedStack, boolean usages, boolean allowItemFallback,
			Optional<JsonObject> focusSelector) {
		try {
			...
			if (usages || isBaseBackpack(focusedStack)) {
				catalog.getGroupedCraftingSpecs().forEach(spec -> {
					...
					displays.stream().map(ReiRecipeViewerAutomation::groupedCraftingRecipeEntry)
			...
			craftingViews.stream().map(ReiRecipeViewerAutomation::craftingDisplayEntry)
					.filter(entry -> containsStack(usages ? entry.inputs() : entry.outputs(), focusedStack, allowItemFallback, focusSelector))
					.forEach(entries::add);
			...
			smithingEntries.stream().filter(entry -> containsStack(usages ? entry.inputs() : entry.outputs(), focusedStack, allowItemFallback, focusSelector))
					.forEach(entries::add);
			return entries;
		} catch (RuntimeException ignored) {
			return List.of();
	...
	private static boolean hasSophisticatedBackpackInput(RecipeEntry entry) {
		return entry.inputs().stream().flatMap(Collection::stream)
				.anyMatch(stack -> "sophisticatedbackpacks".equals(BuiltInRegistries.ITEM.getKey(stack.getItem()).getNamespace()));
	}
	...
	private static RecipeEntry groupedCraftingRecipeEntry(RecipeHolder<GroupedCraftingRecipe> recipeHolder) {
		...
		return new RecipeEntry(recipeHolder.id().identifier().toString(), "minecraft:crafting", "Crafting", recipe.getInputSlots(),
				List.of(recipe.getResultStacks()));
	}

	private static RecipeEntry craftingDisplayEntry(CraftingDisplayView view) {
		return new RecipeEntry(view.spec().id().toString(), "minecraft:crafting", "Crafting", view.spec().getInputSlots(view.variants()),
				List.of(view.spec().getOutputStacks(view.variants())));
	}
	...
	private static List<List<ItemStack>> recipeDisplayInputs(RecipeDisplay display, ContextMap context) {
		...
		if (display instanceof SmithingRecipeDisplay smithing) {
			return List.of(resolveStacks(smithing.template(), context), resolveStacks(smithing.base(), context), resolveStacks(smithing.addition(), context));
		}
		return List.of();
	}
	...
	private static List<RecipeEntry> distinctRecipeEntries(List<RecipeEntry> entries) {
		return entries.stream()
				.collect(Collectors.collectingAndThen(
						Collectors.toMap(ReiRecipeViewerAutomation::recipeEntryKey, entry -> entry, (first, ignored) -> first, LinkedHashMap::new),
						map -> List.copyOf(map.values())));
	}

	private static String recipeEntryKey(RecipeEntry entry) {
		return entry.category() + "|" + RecipeJsonUtil.itemStackIngredientGroupsJson(entry.inputs()) + "|"
	...
	private static RecipeEntry recipeEntry(Display display) {
		return new RecipeEntry(display.getDisplayLocation().map(Identifier::toString).orElse(null), display.getCategoryIdentifier().getIdentifier().toString(),
				categoryName(display), entryIngredients(display.getInputEntries()), entryIngredients(display.getOutputEntries()));
	}

	private static List<List<ItemStack>> entryIngredients(List<EntryIngredient> ingredients) {
		return ingredients.stream()
				.map(ingredient -> ingredient.stream().filter(ReiRecipeViewerAutomation::isItemStack).map(EntryStack::<ItemStack>castValue).toList()).toList();
	}

	private static String recipeJson(RecipeEntry display) {
		return "{" + JsonUtil.property("id", display.id()) + "," + JsonUtil.property("category", display.category()) + ","
				+ JsonUtil.property("categoryName", display.categoryName()) + "," + "\"inputCount\":" + display.inputs().size() + "," + "\"outputCount\":"
				+ display.outputs().size() + "," + "\"inputs\":" + RecipeJsonUtil.itemStackIngredientGroupsJson(display.inputs()) + "," + "\"outputs\":"
				+ RecipeJsonUtil.itemStackIngredientGroupsJson(display.outputs()) + "}";
	}
