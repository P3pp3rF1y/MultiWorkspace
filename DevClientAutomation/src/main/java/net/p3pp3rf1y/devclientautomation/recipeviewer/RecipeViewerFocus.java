...
public final class RecipeViewerFocus {
	...
	private static void putComponent(Item item, CompoundTag tag, String key, JsonElement value) {
		switch (key) {
			...
			case "sophisticatedstorageinmotion:storage_item" ->
				stackFromEncoded(value.getAsJsonObject()).ifPresent(storage -> tag.put("storageItem", storage.save(new CompoundTag())));
