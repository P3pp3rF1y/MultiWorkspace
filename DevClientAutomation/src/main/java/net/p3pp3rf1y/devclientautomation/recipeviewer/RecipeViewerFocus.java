...
public final class RecipeViewerFocus {
	...
	private static void setRenderData(JsonObject components, String key, ItemStack stack) {
		if (components.has(key) && components.get(key).isJsonObject()) {
			...
			RenderData.CODEC.parse(Minecraft.getInstance().level.registryAccess().createSerializationContext(JsonOps.INSTANCE), renderDataJson).result()
	...
	private static void setInt(JsonObject components, String key, IntConsumer setter) {
