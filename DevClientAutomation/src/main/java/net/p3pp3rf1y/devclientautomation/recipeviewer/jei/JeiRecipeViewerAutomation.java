...
public class JeiRecipeViewerAutomation implements RecipeViewerAutomation {
    ...
    public String stateJson() {
        ...
        return "{\"ok\":true,"
                ...
                + JsonUtil.property("searchText", jeiRuntime.getIngredientFilter().getFilterText()) + ","
                + "\"indexStackCount\":" + jeiRuntime.getIngredientManager().getAllItemStacks().size() + ","
                + "\"recipeCount\":" + recipeCount(jeiRuntime) + ","
    ...
    public String queryJson(String requestJson) {
        ...
        return "{\"ok\":true,"
                + JsonUtil.property("viewer", id()) + ","
                + JsonUtil.property("item", itemId(stack.get())) + ","
                + JsonUtil.property("name", stack.get().getHoverName().getString()) + ","
                + "\"uiBacked\":true,"
                + JsonUtil.rawProperty("focusStack", RecipeJsonUtil.itemStackJson(stack.get())) + ","
    ...
    private static Optional<ItemStack> findStack(IIngredientManager ingredientManager, JsonObject request) {
        ...
        Optional<ItemStack> exact = ingredientManager.getAllItemStacks().stream()
                .filter(stack -> itemId(stack).equals(normalized))
                .filter(stack -> focus.map(selector -> RecipeViewerStackMatcher.matches(stack, selector)).orElse(true))
                .findFirst();
        if (exact.isPresent()) {
            return exact;
        }
        return ingredientManager.getAllItemStacks().stream()
                .filter(stack -> stack.getHoverName().getString().toLowerCase(Locale.ROOT).equals(normalized))
                .filter(stack -> focus.map(selector -> RecipeViewerStackMatcher.matches(stack, selector)).orElse(true))
                .findFirst();
    }
    ...
    private static int recipeCount(IJeiRuntime jeiRuntime) {
        return jeiRuntime.getRecipeManager().createRecipeCategoryLookup().get()
                .mapToInt(category -> recipesForCategory(jeiRuntime.getRecipeManager(), category).size())
                .sum();
    }
    ...
    private static List<RecipeEntry> matchingRecipes(IJeiRuntime jeiRuntime, ItemStack stack, RecipeIngredientRole role) {
        ...
        return recipeManager.createRecipeCategoryLookup().limitFocus(List.of(focus)).get()
                .flatMap(category -> recipesForCategory(recipeManager, category, focus).stream().map(recipe -> recipeEntry(recipeManager, category, recipe, focusGroup)))
    ...
    private static RecipeEntry recipeEntry(IRecipeManager recipeManager, IRecipeCategory<?> category, Object recipe, IFocusGroup focusGroup) {
        ...
        if (layout.isPresent()) {
            return new RecipeEntry(recipeId(recipe), category.getRecipeType().getUid().toString(), category.getTitle().getString(), slotStacks(layout.get(), RecipeIngredientRole.INPUT), slotStacks(layout.get(), RecipeIngredientRole.OUTPUT));
        }
    ...
    private static RecipeEntry recipeEntry(IRecipeManager recipeManager, IRecipeCategory<?> category, Object recipe, String id) {
        ...
        return new RecipeEntry(id, category.getRecipeType().getUid().toString(), category.getTitle().getString(),
                ingredients.getIngredients(RecipeIngredientRole.INPUT).stream().map(typed -> typed.getItemStack()).flatMap(Optional::stream).map(List::of).toList(),
                ingredients.getIngredients(RecipeIngredientRole.OUTPUT).stream().map(typed -> typed.getItemStack()).flatMap(Optional::stream).map(List::of).toList());
    }

    private static List<List<ItemStack>> slotStacks(IRecipeLayoutDrawable<?> layout, RecipeIngredientRole role) {
        return layout.getRecipeSlotsView().getSlotViews(role).stream()
                .map(slot -> slotStacks(slot, role))
                .toList();
    }
    ...
    private static List<ItemStack> slotStacks(IRecipeSlotView slot, RecipeIngredientRole role) {
        ...
        slot.getDisplayedItemStack()
                .filter(displayed -> stacks.stream().noneMatch(stack -> ItemStack.isSameItemSameComponents(stack, displayed)))
    ...
    private static int componentScore(List<List<ItemStack>> stacks) {
        return stacks.stream()
                .mapToInt(slot -> slot.stream().mapToInt(JeiRecipeViewerAutomation::componentScore).max().orElse(0))
                .sum();
    }

    private static int componentScore(ItemStack stack) {
        String stackJson = RecipeJsonUtil.itemStackJson(stack);
        return namespaceOccurrences(stackJson, "sophisticatedcore:")
                + namespaceOccurrences(stackJson, "sophisticatedstorage:")
    ...
    private static String recipeJson(RecipeEntry recipe) {
        return "{"
                + JsonUtil.property("id", recipe.id()) + ","
                + JsonUtil.property("category", recipe.category()) + ","
                + JsonUtil.property("categoryName", recipe.categoryName()) + ","
                + "\"inputCount\":" + recipe.inputs().size() + ","
                + "\"outputCount\":" + recipe.outputs().size() + ","
                + "\"inputs\":" + RecipeJsonUtil.itemStackIngredientGroupsJson(recipe.inputs()) + ","
                + "\"outputs\":" + RecipeJsonUtil.itemStackIngredientGroupsJson(recipe.outputs())
                + "}";
    }
    ...
    private static String runtimeUnavailableJson(@Nullable Screen screen) {
        return "{\"ok\":false,"
                + JsonUtil.property("viewer", "jei") + ","
                + JsonUtil.property("error", "JEI runtime is not available") + ","
                + JsonUtil.property("screenClass", screen == null ? null : screen.getClass().getName())
                + "}";
    }
