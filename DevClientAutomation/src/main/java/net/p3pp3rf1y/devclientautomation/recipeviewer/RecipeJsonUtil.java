...
public final class RecipeJsonUtil {
	...
	public static String itemStackJson(ItemStack stack) {
		return "{" + JsonUtil.property("type", "item") + "," + JsonUtil.property("id", itemId(stack)) + ","
				+ JsonUtil.property("name", stack.getHoverName().getString()) + "," + "\"count\":" + stack.getCount() + ","
				+ JsonUtil.property("components", pseudoComponentsJson(stack)) + "," + JsonUtil.rawProperty("componentKeys", componentKeysJson(stack)) + ","
	...
	private static String componentKeysJson(ItemStack stack) {
		...
		return stack.getTag().getAllKeys().stream().map(key -> JsonUtil.property("", key).substring(3)).collect(Collectors.joining(",", "[", "]"));
	}

	private static String encodedStackJson(ItemStack stack) {
		return "{" + JsonUtil.property("id", itemId(stack)) + "," + "\"count\":" + stack.getCount() + ","
	...
	static String pseudoComponentsJson(ItemStack stack) {
		...
		if (tag.contains("storageItem", Tag.TAG_COMPOUND)) {
			addComma(json, first);
			json.append(
					JsonUtil.property("sophisticatedstorageinmotion:storage_item", "").substring(0, "\"sophisticatedstorageinmotion:storage_item\":".length()))
