    ...
    foreach ($item in $items) {
        ...
        if ($item -eq "barrel" -or $item -eq "barrel_directional" -or $item -eq "limited_barrel_3_directional") {
            ...
            foreach ($targetName in @("top", "side", "bottom")) {
                ...
                if ($item -eq "barrel_directional" -and $targetName -eq "top") {
                    $transitionStartScreenshotPath = Join-Path $ScreenshotDirectory "$item-$targetName-transition-start.png"
                    Invoke-WebRequest -Uri "$BaseUrl/screenshot" -OutFile $transitionStartScreenshotPath | Out-Null
                    $transitionStartCropPath = Join-Path $rotationCropDirectory "$item-$targetName-transition-start.png"
                    Save-DecorationTablePreviewCrop -ScreenshotPath $transitionStartScreenshotPath -PreviewCropPath $transitionStartCropPath -State (Invoke-BridgeJson -Method Get -Path "/state") -Preview $open.preview
                    $transitionStartHashDistance = Get-HammingDistance -First (Get-CenteredPerceptualHash -ScreenshotPath $rotationBaselineCropPath) -Second (Get-CenteredPerceptualHash -ScreenshotPath $transitionStartCropPath)
                    Assert-True ($transitionStartHashDistance -le 24) "Directional barrel material hover changed its starting orientation. Perceptual hash distance=$transitionStartHashDistance Crop=$transitionStartCropPath"
                }
            ...
            if ($item -eq "barrel_directional") {
                $returnHashDistance = Get-HammingDistance -First (Get-CenteredPerceptualHash -ScreenshotPath $rotationBaselineCropPath) -Second (Get-CenteredPerceptualHash -ScreenshotPath $returnedCropPath)
                Assert-True ($returnHashDistance -le 24) "Directional barrel material hover did not return to its default orientation. Perceptual hash distance=$returnHashDistance Crop=$returnedCropPath"
            }
